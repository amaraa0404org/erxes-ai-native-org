...
- [x] **UI-10**: Every consumer (other plugin UI) sees a clean error if AI is disabled — `useAIAvailable()` hook + `<ErrorBoundary>` at every `ai_ui` remote import; no MF hard-fail on missing remote

### MF Hooks & Reference Integrations (HOOK)

- [ ] **HOOK-01**: `ai_ui` Module Federation exposes `./config`, `./Copilot`, `./atoms`, `./hooks`, `./widgets`
- [x] **HOOK-02**: `shared` config declares `react`, `react-dom`, `@apollo/client`, `jotai`, `erxes-ui` as `singleton: true` with `requiredVersion` matched to root `package.json`
...
- [x] **XCUT-04**: `pnpm install && pnpm nx build erxes-api-shared && pnpm nx build ai_api && pnpm nx build ai_ui` succeed from a clean clone
...
- [x] **XCUT-10**: `ai_api/Dockerfile` uses `node:18.20-alpine`; `docker-compose.yml` gains an optional `qdrant` service; `.env.sample` lists every new variable
...
| UI-10 | Phase 1.1 | Complete |
| HOOK-01 | Phase 3.2 | Pending |
| HOOK-02 | Phase 1.1 | Complete |
